          ...
          <div class="card-body">
            <table id="creators" class="table table-bordered table-striped">
              ...
              <tr>
                ...
                <td>{{ $creator->media_url }}</td>
                <td>
                  <a class="updateUsertatus" href="javascript:void(0)" creator_id="{{ $creator->id }}" id="creator-{{ $creator->id }}">
                ...
                <td>
                  <a href="{{ url('admin/edit_creator/'.$creator->id) }}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit creator">
                    <i class="fas fa-user-edit text-secondary text-sm " data-bs-toggle="tooltip" data-bs-placement="top" title="edit creator"></i>
                  </a>
                  <form action="{{ url('admin/delete_creator/'.$creator->id) }}" method="post" class="d-inline">
                    @method('DELETE')
                    @csrf
                    <input type="hidden" name="_method" value="DELETE">
                  <button type="submit"  style=" background: none;padding: 0px; border: none" class="confirm_creator_delete">
                    <i class="fas fa-trash text-secondary text-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="delete creator" ></i>
                  </button>
                  </form>
                </td>
