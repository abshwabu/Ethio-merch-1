				...
				<div class="product product-style-2 equal-elem ">
					<div class="product-thumnail">
						<a href="{{route('product.details',['slug'=>$sproduct->slug])}}"
							title="{{$sproduct->product_name}}">
							<figure><img src="{{ asset('storage/'. $sproduct->product_image ) }}" width="800"
					...
					<div class="product-info">
						<a href="#" class="product-name"><span>{{$sproduct->product_name}}</span></a>
						<div class="wrap-price"><ins>
								<p class="product-price">${{$sproduct->sale_price}}</p>
							</ins><del class="product-price">${{$sproduct->regular_price}}</del> </div>
					</div>
				</div>
				@endforeach
								...
								<div class="product product-style-2 equal-elem ">
									<div class="product-thumnail">
										<a href="{{route('product.details',['slug'=>$latestproduct->slug])}}"
											title="{{$latestproduct->product_name}}">
											<figure><img src="{{asset('storage/'.$latestproduct->product_image)}}"
									...
									<div class="product-info">
										<a href="#"
											class="product-name"><span>{{$latestproduct->product_name}}</span></a>
										<div class="wrap-price"><span
												class="product-price">${{$latestproduct->regular_price}}</span></div>
									</div>
								</div>
								...
								<div class="product product-style-2 equal-elem ">
									<div class="product-thumnail">
										<a href="{{route('product.details',['slug'=>$c_product->slug])}}"
											title="{{$c_product->product_name}}">
											<figure><img src="{{asset('storage/'.$c_product->product_image)}}"
													width="800" height="800" alt="{{$c_product->product_name}}">
											</figure>
										</a>
									...
									<div class="product-info">
										<a href="{{route('product.details',['slug'=>$c_product->slug])}}"
											class="product-name"><span>{{$c_product->product_name}}</span></a>
										<div class="wrap-price"><span
