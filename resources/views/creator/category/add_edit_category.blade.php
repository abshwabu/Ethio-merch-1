...
<li class="breadcrumb-item"><a href="{{ url('creator/categories') }}">Categories</a></li>
@endsection
        ...
        <div class="card">
          ...
          <div class="card-body">
            <form method="POST" role="form" class="text-start" @if (empty($categoryData))
              action="{{ url('creator/add-edit-category') }}" @else
              action="{{ url('creator/add-edit-category/'.$categoryData->id) }}" @endif enctype="multipart/form-data">
              @csrf
              <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="form-group">
                      ...
                      <select name="section_id" id="section_id" class="form-control select2" style="width: 100%"
                        ...
                        <option value="{{ $section->id }}" @if(!empty($categoryData->section_id) &&
                          $categoryData->section_id==$section->id) selected @elseif(!empty(@old('section_id')) &&
                          @old('section_id')==$section->id) selected @endif>{{ $section->name }}</option>
                        @endforeach
                      </select>
                    </div>
                    @error('section_id')
                  ...
                  {{-- <div class="col-md-6 my-2" id="appendCategoriesLevel">
                    <div class="form-group">
                      @include('creator.category.append_categories_level')
                    </div>
                    @error('parent_id')
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $message }}</strong>
                  ...
                  <div class="col-md-6 my-2">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control @error('cate_discount') is-invalid @enderror"
                        name="cate_discount" @if (!empty($categoryData->cate_discount))
                      value="{{$categoryData->cate_discount }}" @else value="0" @endif>
                  ...
                  <div class="col-md-6 my-2 form-group">
                    ...
                    <label>slug</label>
                    <input type="text" class="form-control @error('slug') is-invalid @enderror gfaadagdfgadg"
                      name="slug" @if (!empty($categoryData->slug)) value="{{ $categoryData->slug }}"
