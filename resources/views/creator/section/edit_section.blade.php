        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <form method="POST" role="form" class="text-start"
              action="{{ url('creator/section_update/'.$section->id) }}" enctype="multipart/form-data">
              @csrf
              <div class="my-3 col-md-6 mx-auto">
                <label>name</label>
                <input type="text " class="form-control @error('name') is-invalid @enderror" value="{{$section->name}}"
                  name="name">
                @error('name')
